  ...
  mDragging(false)
{
  ...
  ////---------------EXTENDED BY HEHUI---------------////
  mCrossCurveVisible = false;
  mCrossCurvePoint = QPointF(0, 0);
  ////---------------EXTENDED BY HEHUI---------------////
...
void QCPAxisRect::setCrossCurveVisible(bool visible){
...
bool QCPAxisRect::pointToCoords(const QPoint &pos, double &key, double &value, QCPAxis::AxisType xAxisType, int xAxisIndex, QCPAxis::AxisType yAxisType, int yAxisIndex){
    ...
    QCPAxis *yAxis = axis(yAxisType, yAxisIndex);
    if(!xAxis || !yAxis){return false;}

    int x = pos.x();
    int y = pos.y();
    key = xAxis->pixelToCoord(x);
    value = yAxis->pixelToCoord(y);
    return true;
}
...
void QCPAxisRect::draw(QCPPainter *painter)
{
  drawBackground(painter);
  ////---------------EXTENDED BY HEHUI---------------////
  if(mCrossCurveVisible){
      //Draw Cross Curve
      painter->setPen(QPen(QColor(10, 40, 180)));
      double x = mCrossCurvePoint.x();
      double y = mCrossCurvePoint.y();
      if(y >= mRect.top() && y <= mRect.bottom()){
          //Horizontal Line
          painter->drawLine(QPointF(mRect.left(), y), QPointF(mRect.right(), y));

          QCPAxis *yAxis = axis(QCPAxis::atLeft, 0);
          if(yAxis){
              double value = yAxis->pixelToCoord(y);
              char buffer[65];
              sprintf(buffer, "%.2f", value);

              QFont ft = QApplication::font();
              ft.setBold(true);
              ft.setPointSize(ft.pointSize()*1.25);
              painter->setFont(ft);
              QFontMetrics fm(ft);
              int pixelsValueStringWide = fm.width(QString(buffer));
              QCPAxis *ax = axis(QCPAxis::atRight);
              if(ax){
                  pixelsValueStringWide = ax->calculateMargin();
              }

              int pixelsHigh = fm.height();
              QRectF textRect(QPointF(mRect.right()+2, y-pixelsHigh/2), QSize(pixelsValueStringWide*1.2, pixelsHigh*1.2));

              painter->setBrush(QBrush(QColor(0, 255, 255)));
              painter->drawRect(textRect);

              painter->setPen(QPen(QColor(10, 40, 180)));
              painter->drawText(textRect, Qt::AlignHCenter|Qt::AlignVCenter, QString(buffer));
          }
      }
      if(x >= mRect.left() && x <= mRect.right()){
          //Vertical Line
          painter->drawLine(QPointF(x, mRect.top()), QPointF(x, mRect.bottom()));
      }
  }
  ////---------------EXTENDED BY HEHUI---------------////
}
...
void QCPPlotTitle::setAlignmentFlag(Qt::AlignmentFlag flag){
    mAlignmentFlag = flag;
}
...
void QCPPlotTitle::draw(QCPPainter *painter)
{
  ...
  painter->drawText(mRect, mAlignmentFlag, mText, &mTextBoundingRect);
}
...
*/
void QCPFinancial::drawCandlestickPlot2(QCPPainter *painter, const QCPFinancialDataMap::const_iterator &begin, const QCPFinancialDataMap::const_iterator &end)
{
  QCPAxis *keyAxis = mKeyAxis.data();
  QCPAxis *valueAxis = mValueAxis.data();
  if (!keyAxis || !valueAxis) { qDebug() << Q_FUNC_INFO << "invalid key or value axis"; return; }
...
void QCPFinancial::getBoundValuesInVisibleRange(double &minValue, double &maxValue, uint &itemCount, double *leftKey, double *rightkey) const
{
  ...
  {
      ++itemCount;
      if(it.value().open == 0){continue;}

      double currentLow = it.value().low;
      double currentHigh = it.value().high;
  ...
  if(leftKey){
      ...
      *leftKey = lower.value().key;
  }

  if(rightkey){
      //*rightkey = upper.value().key+mWidth*0.5;
      *rightkey = upper.value().key;
  }
...
void QCPFinancial::getBoundValuesInVisibleRange(QCPRange &valueRange, QCPRange &leftBoxRange, QCPRange &rightBoxRange) const
{
  ...
  leftBoxRange.lower = lower.value().key-mWidth*0.5;
  leftBoxRange.upper = lower.value().key+mWidth*0.5;

  rightBoxRange.lower = upper.value().key-mWidth*0.5;
  rightBoxRange.upper = upper.value().key+mWidth*0.5;
...
bool QCPFinancial::pointToCoords(const QPointF &pos, double &key, double &value) {
    ...
    QCPFinancialDataMap::const_iterator begin, end; // note that upper is the actual upper point, and not 1 step after the upper point
    getVisibleDataBounds(begin, end);
    if (begin == mData->constEnd() || end == mData->constEnd())
    {
...
void QCPFinancial::drawCandlestickPlot(QCPPainter *painter, const QCPFinancialDataMap::const_iterator &begin, const QCPFinancialDataMap::const_iterator &end)
{
  ...
  if (keyAxis->orientation() == Qt::Horizontal)
  {
    for (QCPFinancialDataMap::const_iterator it = begin; it != end; ++it)
    {
      ...
      //TODO:if(it.value().open == 0){continue;}
      double currentLow = it.value().low;
      double currentHigh = it.value().high;
      if (currentHigh >= maxYValue){
          maxYValue = currentHigh;
          keyOfmaxYValue = it.value().key;
      }
      if(currentLow <= minYValue){
          minYValue = currentLow;
          keyOfMinYValue = it.value().key;
      }
